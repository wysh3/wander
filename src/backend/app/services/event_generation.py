...
async def generate_local_events(
    ...
    try:
        ...
        async def _search_area(area: str) -> list[tuple[str, str]]:
            chunks: list[tuple[str, str]] = []
            query = f"upcoming events concerts workshops meetups this weekend in {area} Bangalore"
            ...
            try:
                ...
                for text in results:
                    ...
                    chunks.append((area, tagged))
                logger.info("exa_search_done", area=area, results=len(results))
            except Exception as e:
                logger.warning("exa_search_error", area=area, error=str(e))
            return chunks

        area_results = await asyncio.gather(*[_search_area(a) for a in selected_areas])
        for area_chunks in area_results:
            text_chunks_by_area.extend(area_chunks)
            total_sources += len(area_chunks)
