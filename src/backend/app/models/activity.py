...
class Activity(Base):
    ...
    # ── Admin Panel / Local Events extension ──
    is_local_event: Mapped[bool] = mapped_column(Boolean, default=False)
    cover_photo_url: Mapped[str | None] = mapped_column(Text)
    min_capacity: Mapped[int] = mapped_column(Integer, default=4)
    max_capacity: Mapped[int] = mapped_column(Integer, default=50)
    ticket_type: Mapped[str] = mapped_column(String(20), default="free")
    ticket_price_inr: Mapped[int] = mapped_column(Integer, default=0)
    visibility: Mapped[str] = mapped_column(String(20), default="public")
    invite_token: Mapped[str | None] = mapped_column(String(64))
    host_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    tags: Mapped[list[str] | None] = mapped_column(ARRAY(Text), default=[])
