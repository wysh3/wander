    ...
    async with async_session() as session:
        ...
        activities = [
            ...
            Activity(title="TCS World 10k Marathon Training",
                     ...
                     category="physical", area="Kanteerava",
                     lat=12.9692, lng=77.5943,
                     scheduled_at=datetime.utcnow() + timedelta(days=5, hours=5, minutes=30),
                     group_size_min=5, group_size_max=20, max_groups=4,
                     host_ids=[host_users[4].id], status="open",
                     is_local_event=True,
                     cover_photo_url="https://images.unsplash.com/photo-1552674605-db6ffd4facb5?w=800&q=80",
                     tags=["marathon", "running", "sports", "fitness", "endurance"]),
            
            Activity(title="Hackverse-2k26 Code-athon",
                     description="24-hour hackathon focused on AI for Social Impact. Build something meaningful.",
                     category="skill", area="Doddaballapura",
                     lat=13.2924, lng=77.5385,
                     scheduled_at=datetime.utcnow() + timedelta(days=7, hours=10),
                     group_size_min=4, group_size_max=50, max_groups=10,
                     host_ids=[host_users[0].id], status="open",
                     is_local_event=True,
                     cover_photo_url="https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=800&q=80",
                     tags=["hackathon", "coding", "ai", "technology", "social_good"]),
            
            Activity(title="Robin Hood Army Food Drive",
                     description="Distributing surplus food to underprivileged communities across Indiranagar.",
                     category="social_good", area="Indiranagar",
                     lat=12.9716, lng=77.6411, venue_id=venues[3].id,
                     scheduled_at=datetime.utcnow() + timedelta(days=4, hours=19),
                     group_size_min=5, group_size_max=15, max_groups=2,
                     host_ids=[host_users[1].id], status="open",
                     is_local_event=True,
                     cover_photo_url="https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
                     tags=["ngo", "social_good", "volunteering", "community", "charity"]),
            # ----------------------------
